...
func (r *Runtime) cleanPlot(
	...
	*plot = Plot{ID: clean.PlotId, State: plotv1.PlotState_EMPTY}
	...
	payload := &wsv1.CleanPlotResponse{
...
func replayCleanPlot(
	...
	if stored.Success {
		...
		if proto.Unmarshal(stored.ResponsePayload, payload) == nil {
			response.Payload = &wsv1.WsEnvelope_CleanPlotResponse{CleanPlotResponse: payload}
			return response
		}
	} else {
		failure := &wsv1.Error{}
...
func cleanPlotFingerprint(
	callerPlayerID uint64,
	envelope *wsv1.WsEnvelope,
	request *wsv1.CleanPlotRequest,
) [sha256.Size]byte {
	body := make([]byte, 0, 32)
