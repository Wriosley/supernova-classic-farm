...
当前生产目标以 [V3 有状态 Zone](../../architecture/stateful-zone-v3-architecture.md) 为准；第一条闭环以 [单玩家业务架构](../../architecture/single-player-vertical-loop-business-architecture.md) 为准。本文仍包含 V1/V2 时期的模块和数据讨论，只作为导航与历史参考；其中 HTTP 业务命令、数据库实时事实、行锁主流程、同步 Journal、异步 Task Service 或自动领奖等旧语义不得覆盖当前文档和最新 accepted ADR。
...
“本地原型”是可演示和可故障验证的缩小实现，不等于 3000 万 DAU 生产系统。当前生产目标见 [V3](../../architecture/stateful-zone-v3-architecture.md)；同步 Journal V2 和无状态 V1 只作为历史对照。容量目标、候选中间件和规划实例数不能写成已具备能力。
...
| 任务 | Player Actor 内章节进度、达标、手动领取和奖励记录 | 第一阶段末 |
...
最早曾计划使用 HTTP JSON、模块化单体和本地数据库事务打通业务。ADR-0001 的代码模块边界仍可复用，但当前原型必须经过 V3 的 WebSocket、Player Actor 和 Dirty 写回路径，不能实现成另一套数据库实时事实架构。
...
生产目标采用 [V3](../../architecture/stateful-zone-v3-architecture.md)：GateSvr 路由到逻辑 Shard 的 Active Zone Owner；Player Actor 串行修改内存并立即响应；统一 Flusher 异步批量保存 MySQL 检查点。生产 Coordinator 使用多数派授权、租约和 epoch fencing，本地实现接口兼容的单节点原型。
...
| 任务 | 配置、当前章节进度、完成/领取状态、奖励记录 | Actor 内同步推进、手动领取、查询 | 客户端不能上报进度或奖励 |
...
- 任务：配置由 ConfigSvr 提供；当前章节进度、`CLAIMABLE` 和领取状态属于 Player Actor。达标后手动领取，满仓物品生成邮件 Outbox。
...
### 7.2 购买、种植、施肥、成长、收获、出售和任务

完整规则、状态机、Buff 时间模型、章节任务和失败语义见 [单玩家纵向闭环业务架构](../../architecture/single-player-vertical-loop-business-architecture.md)。这些动作在同一 Player Actor 内串行执行并同步推进当前章节任务，成功后标记 Dirty；MySQL 不在在线命令同步路径。
...
- 生产目标以有状态 Player Actor Zone 和 4096 个逻辑分片为核心；账号、好友、邮件、实时和归档按流量与一致性特征拆分。
- 好友/邀请：30 分钟随机 Token 可由不同玩家多次使用，普通玩家最多 200 名好友；关系、邀请和访问授权由好友服务拥有。
- 当前章节任务：属于 Player Actor，与成功业务动作同步推进；全服、排行榜或跨玩家任务未来再评审独立服务。
- 邮件：满仓偷菜奖励必须转为不自动过期的系统奖励邮件；完整运营邮件能力后续扩展。
- 多人同步：游戏命令、快照和推送统一经过 Client 到 GateSvr 的 WebSocket；版本缺口触发补发或完整快照。
- 容量、分片、Dirty、所有权、恢复和原型验证见 [V3 目标架构](../../architecture/stateful-zone-v3-architecture.md)。
## 13. 测试与证据
...
当前规划假设为 3000 万 DAU、375 万正常峰值在线、约 500 万峰值驻留 Actor、约 6.94 万正常规划峰值游戏消息和 4096 个逻辑 Shard；中档压测前设计点为约 60 个 Zone。目标采用有状态 Player Actor、异步 Dirty 检查点和多数派授权 Owner。以上均是规划值而非实测能力，完整边界见 [V3](../../architecture/stateful-zone-v3-architecture.md)。
## 15. 开发顺序与完成标准

1. **目标设计基线**：容量模型、服务边界、玩家分片、可靠事件、实时同步、可用性和原型范围完成书面评审；
2. **单玩家 Actor 原型**：最小 Go/Vue 骨架，完成购买、种植、收获、出售、命令幂等、事件决定与内存应用；
3. **持久化与所有权原型**：Dirty 批量写回、MySQL 恢复、两个有状态 Zone、4096 逻辑分片路由、epoch fencing 和迁移；
4. **异步原型**：跨玩家偷菜奖励、满仓邮件和重复消息验证；
...
- 当前章节任务在 Player Actor 内同步更新；跨玩家奖励可靠异步处理；满仓奖励转系统邮件。
- HTTP 负责注册登录和 Gateway 发现；游戏命令、快照和推送使用 WebSocket。
...
- Event Bus 仅作为 Outbox 下游候选，不是玩家恢复 Journal；
- 种植时固化成长阈值和基础速度，动态 Buff 下保存累计成长和效果区间；
- 顶层 `request_id` + 业务唯一键 + Outbox/幂等消费者。

### 16.3 实施前仍待确认

1. Redis 与替代缓存的原型复杂度和压测结果；
2. Dirty 批次、检查点大小、MySQL Shard 和 epoch fencing 的压测参数；
3. 初始地块数和第二章任务数值；
...
- 当前 V3 目标架构：`docs/architecture/stateful-zone-v3-architecture.md`
- 第一条业务闭环：`docs/architecture/single-player-vertical-loop-business-architecture.md`
- 同步 Journal V2 历史：`docs/archive/architecture-v1-v2/stateful-zone-v2-architecture.md`
- 无状态 V1 历史方案：`docs/archive/architecture-v1-v2/target-30m-dau-architecture.md`
- 模块设计、接口能力与数据流：`docs/architecture/module-design-and-flows.md`
- 稳定项目事实：`docs/context/PROJECT.md`
- 当前工作交接：`docs/context/CURRENT.md`
- 架构决策：`docs/decisions/`
- 实施计划：`docs/archive/development/plans/`
- 测试与压测证据：`docs/evidence/`
- AI 工作记录：`docs/archive/development/ai-workflow/`
