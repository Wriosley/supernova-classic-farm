...
当前生产目标以 [V3 有状态 Zone](stateful-zone-v3-architecture.md) 为准；第一条闭环以 [单玩家业务架构](single-player-vertical-loop-business-architecture.md) 为准。本文仍包含 V1/V2 时期的模块和数据讨论，只作为导航与历史参考；其中 HTTP 业务命令、数据库实时事实、行锁主流程、同步 Journal、异步 Task Service 或自动领奖等旧语义不得覆盖当前文档和最新 accepted ADR。
...
“本地原型”是可演示和可故障验证的缩小实现，不等于 3000 万 DAU 生产系统。当前生产目标见 [V3](stateful-zone-v3-architecture.md)；同步 Journal V2 和无状态 V1 只作为历史对照。容量目标、候选中间件和规划实例数不能写成已具备能力。
...
生产目标采用 [V3](stateful-zone-v3-architecture.md)：GateSvr 路由到逻辑 Shard 的 Active Zone Owner；Player Actor 串行修改内存并立即响应；统一 Flusher 异步批量保存 MySQL 检查点。生产 Coordinator 使用多数派授权、租约和 epoch fencing，本地实现接口兼容的单节点原型。
...
完整规则、状态机、Buff 时间模型、章节任务和失败语义见 [单玩家纵向闭环业务架构](single-player-vertical-loop-business-architecture.md)。这些动作在同一 Player Actor 内串行执行并同步推进当前章节任务，成功后标记 Dirty；MySQL 不在在线命令同步路径。
...
- 容量、分片、Dirty、所有权、恢复和原型验证见 [V3 目标架构](stateful-zone-v3-architecture.md)。
## 13. 测试与证据
...
当前规划假设为 3000 万 DAU、375 万正常峰值在线、约 500 万峰值驻留 Actor、约 6.94 万正常规划峰值游戏消息和 4096 个逻辑 Shard；中档压测前设计点为约 60 个 Zone。目标采用有状态 Player Actor、异步 Dirty 检查点和多数派授权 Owner。以上均是规划值而非实测能力，完整边界见 [V3](stateful-zone-v3-architecture.md)。
## 15. 开发顺序与完成标准
...
- 同步 Journal V2 历史：`docs/architecture/stateful-zone-v2-architecture.md`
