...
本文同时描述稳定业务语义和实现边界。本地单进程事务只用于原型第一阶段；生产目标中的玩家分片、任务异步、跨分片偷菜、邮件兜底和实时同步以 [3000 万 DAU 目标架构](target-30m-dau-architecture.md) 与 ADR-0002 为准。
...
- **已确认规则**：不提供手动领取；第一批任务只发金币。Zone 在核心事务内写行为 Outbox；任务服务异步更新进度、自动完成并以唯一 `reward_id` 发奖。任务故障不回滚种植、收获或交易，客户端允许短暂显示同步中。
- **不变量**：只有已提交业务事件增加进度；`event_id` 防重复进度，任务状态条件更新防重复完成，`reward_id` 防重复发奖；客户端不能指定进度或奖励。
