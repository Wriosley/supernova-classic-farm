...
- **注册失败语义**：`account + player + wallet + farm + initial_plots` 在同一事务创建，任一步失败则整体回滚，对外表现为“注册失败且账号不存在”。新手任务和欢迎邮件不进入该事务。
...
- **数据**：农场、地块、作物配置、种植/产出快照、偷菜信息、种植与收获历史、版本。
- **主流程**：获取快照；种植；按时间派生成熟；农场主收获；好友偷菜。
- **已确认规则**：农场主收获后清空地块并保存种植/收获历史；每轮成熟作物最多被好友偷取一次；演示基准产量为 10，偷菜者获得 3，农场主剩余 7；偷菜不清空地块。
- **不变量**：一块地只有一轮种植；成熟前不能收获或偷取；一轮只由农场主收获一次；偷菜者必须是农场主的好友且不能是本人；同一地块上的偷菜与收获串行校验；地块、相关资产、历史和幂等结果同事务。

### 3.5 任务

- **目标**：把成功业务行为转成进度，达标时自动完成和发奖。
- **数据**：任务配置、玩家进度、完成状态、奖励快照、奖励记录、行为去重记录。
- **主流程**：接收成功行为；更新进度；达标后自动完成并发放奖励；查询任务和奖励记录。
- **已确认规则**：第一阶段不提供手动领取；只发金币奖励；任务进度、完成状态、金币和奖励记录加入触发该行为的同一事务，任一步失败则核心动作回滚。
- **不变量**：失败业务不增加进度；同一业务事实只贡献一次；同一任务只完成和发奖一次；客户端不能指定进度或奖励。
...
- **不变量**：不能添加自己；两人最多一条关系；关系对称；重复接受返回已有关系；过期或已撤销邀请不可使用；邀请生成需要限频。
- **待确认**：删除好友是否进入第一版。
...
- **阶段状态**：最终范围保留，本阶段不开发；邮件来源、有效期、管理员发信和删除规则后续确认。
...
| 农场 | 好友偷菜 | 登录 | 目标地块、`request_id` | 偷取数量、地块剩余产量与版本 | 非好友、未成熟、已偷取、仓库超限、并发冲突 |
...
→ 校验资产格数/堆叠并增加种子
→ 写交易、资产流水和幂等结果
→ 调用任务 RecordAction；若达标则自动发金币并记录奖励
→ 一并提交
```
...
任务没有手动领奖写接口。农场或商店用例在自己的事务中调用 `RecordAction`：先按业务事实去重并更新进度；达到目标时把状态改为已完成、增加金币、保存奖励快照和可查询奖励记录。任一步失败都回滚触发它的核心动作。后续出现性能证据后，才讨论可靠事件和最终一致性。
...
→ 校验好友关系并锁定成熟地块
→ 复核本轮尚未被偷，固化本轮偷取量（演示：10 的 30%，即 3）
→ 校验偷菜者资产容量并增加作物
→ 地块写 stolen_quantity、stolen_by、stolen_at、历史和递增版本
→ 写幂等结果并提交
→ 提交后实时广播
```

偷菜不清空地块。农场主与好友并发操作同一地块时都先锁同一行：农场主先提交则好友得到“已收获”；好友先提交则农场主随后收获剩余 7。实时模块不能直接写农田数据库。
...
- 资产、地块、偷菜状态、任务完成/奖励记录等强一致数据必须在同一事务。
- 事务内禁止 WebSocket 广播和其他外部网络调用；只在提交后广播。
- 注册核心初始化失败整体回滚；宠物、图鉴、邮件不进入当前阶段的核心事务。

### 7.3 容量、幂等与并发

- 物品数量为 0 不占格；新增第 101 种物品或单种超过 300 时，整个业务失败。
- 顶层写操作用 `request_id` 重放已提交结果；资产流水和任务行为使用稳定业务编号防内部重复。
