...
2026-09-13 界面简化：使用默认 Qt 控件与文字；在地块列表中选择一块地，再点击种植、施肥、收获或清理。库存不足、尚未成熟、任务未完成等提示直接取自服务器。`farmwindow.cpp` 只保留构造和刷新两个函数；登录、邮箱的短按钮回调直接写在构造函数里。网络仍单独放在 `farmapiclient.cpp`，未确认操作仍以原编号重试。安装 Qt 后已编译并启动窗口，业务操作尚未复测，见 [验证记录](evidence/2026-09-13-qt-simple.md)。
