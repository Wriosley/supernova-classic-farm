...
Phase 06 Tasks 1–5 and one live Tcaplus migration/restart gate are verified.
The per-persisted-step live Coordinator kill matrix remains pending, so the
Kubernetes worker switch remains disabled by default.
...
- The live test covered successful migration plus post-completion Coordinator
  restart. It did not kill Coordinator at every intermediate persisted step.
