...
stack_pid=""
run_root="$(mktemp -d "${TMPDIR:-/tmp}/classic-farm-friend-e2e.XXXXXX")"
...
show_stack_log() {
    if [[ -f "${stack_log}" ]]; then
        echo "----- Friend E2E backend stack log -----" >&2
...
echo "PHASE friend-interaction owner=${owner_account}"
start_stack
run_friend_interaction
stop_stack

e2e_passed=true
echo "RESULT friend_interaction=PASS owner=${owner_account}"
